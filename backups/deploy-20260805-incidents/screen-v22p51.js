  ...
  async function renderTimeOff(main) {
    ...
    main.appendChild(html(`
      <div style="padding:24px;max-width:1800px;margin:0 auto;">
        ...
        <button id="tor-new" class="btn btn-primary" style="margin-top:6px;background:linear-gradient(135deg,#0FA3B1,#1F6080);color:#fff;border:0;font-weight:800;cursor:pointer;box-shadow:0 8px 18px -8px rgba(31,96,128,.55);${isMobile()?'display:block;width:100%;padding:14px;font-size:15px;border-radius:13px;':'padding:11px 20px;border-radius:11px;font-size:14px;'}">🌴 Request time off</button>
  ...
  async function renderPayroll(main) {
    main.innerHTML = `
      <div style="padding:24px;max-width:1800px;margin:0 auto;">
        ...
        <div style="margin-top:32px;background:#fff;border:1px solid #E5E7EB;border-radius:12px;padding:20px;max-width:720px;">
          ...
          <div style="font-size:11.5px;color:#94A3B8;margin-top:4px;">Leave blank to use the standard "Private &amp; Confidential" notice.</div>
    ...
    // ── Payslip appearance (agency-wide wording on the payslip PDF) ──────
    (function () {
      var noteEl = document.getElementById('ps-note'), confEl = document.getElementById('ps-conf'),
          saveBtn = document.getElementById('ps-save'), statusEl = document.getElementById('ps-status');
      if (!noteEl || !saveBtn) return;
