...
(function (window) {
  ...
  function ldTxnCard(row) {
    ...
    return '<div class="kt-ldx"' + (isInv ? ' data-inv="' + row.invoice_id + '" style="cursor:pointer;' : ' style="') + 'display:flex;align-items:center;gap:11px;background:#fff;border:1px solid #EDF1F6;border-radius:14px;padding:12px 13px;margin-bottom:9px;box-shadow:0 1px 4px rgba(15,23,42,.05);">'
      ...
      +   '<div style="font-weight:700;font-size:14px;color:#0F172A;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + esc(row.description) + (isInv ? ' <span style="color:#2C7BA0;font-size:11px;font-weight:800;">↗</span>' : '') + '</div>'
      +   '<div style="font-size:11.5px;color:#94A3B8;margin-top:2px;">' + esc(fmtDate(row.date)) + late + '</div>'
      + '</div>'
      + '<div style="flex:0 0 auto;text-align:right;">'
      +   '<div style="font-weight:800;font-size:14.5px;color:' + amtColor + ';white-space:nowrap;">' + amt + '</div>'
      +   '<div style="font-size:11px;color:#94A3B8;margin-top:2px;white-space:nowrap;">bal ' + fmtMoney(row.running_balance) + '</div>'
      + '</div>'
      + '</div>';
  }
  async function renderLedger(main) {
    ...
    const txnHtml = rows.length
      ? (ldIsMobile()
          ...
          : `<div class="kt-card"><table>
              ...
              <tbody>${rows.map(row => `<tr${row.type === 'invoice' && row.invoice_id ? ` data-inv="${row.invoice_id}" style="cursor:pointer;"` : ''}>
                <td>${fmtDate(row.date)}</td>
                <td style="text-transform:capitalize;">${esc(row.type)}</td>
                <td>${esc(row.description)}${row.type === 'invoice' && row.invoice_id ? ' <span style="color:#2C7BA0;font-size:11px;font-weight:700;white-space:nowrap;">↗ view</span>' : ''}${row.days_late > 0 ? ' <span style="display:inline-block;margin-left:6px;padding:1px 7px;border-radius:10px;background:#FEE2E2;color:#B91C1C;font-size:11px;font-weight:700;white-space:nowrap;">' + row.days_late + ' day' + (row.days_late === 1 ? '' : 's') + ' late</span>' : ''}</td>
    ...
    main.querySelectorAll('[data-inv]').forEach(elx => {
      elx.addEventListener('click', () => {
        const id = elx.getAttribute('data-inv');
        if (id && window.KT && KT.openInvoiceById) KT.openInvoiceById(id);
      });
    });
