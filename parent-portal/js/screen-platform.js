    ...
    Api.get('/platform/agencies').then(function (data) {
      ...
      (data.agencies || []).forEach(function (a) {
        ...
        var resendBtn = Dom.el('button', {
          style: 'background:transparent;color:#0E7490;border:1px solid #0E7490;padding:5px 10px;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;margin-right:4px;',
        }, '✉ Resend');
    ...
    function render() {
      ...
      if (s === 1) {
        bodyHtml =
          ...
          row('Agency logo', '<div style="display:flex;align-items:center;gap:12px;">' +
              '<div id="w-logo-prev" style="width:96px;height:60px;border:1px solid #E5E7EB;border-radius:8px;background:#F9FAFB ' + (st.logoUrl ? "url('" + esc(absUrlP(st.logoUrl)) + "')" : '') + ' center/contain no-repeat;display:flex;align-items:center;justify-content:center;color:#9CA3AF;font-size:22px;flex-shrink:0;">' + (st.logoUrl ? '' : '🖼') + '</div>' +
              ...
              '<span id="w-logo-msg" style="font-size:12px;color:#9CA3AF;">PNG/SVG, max 2 MB</span>' +
            '</div>', true) +
      ...
      modal.innerHTML =
        '<div style="padding:22px 24px 0;"><h3 style="margin:0 0 4px;font-size:19px;">🌐 Create new agency</h3>' +
          '<div style="font-size:12px;color:#9CA3AF;margin-bottom:16px;">Step ' + s + ' of 4</div>' + stepDots(s) + '</div>' +
        ...
        '<table style="width:100%;border-collapse:collapse;font-size:13px;">' +
          ...
          '<tbody>' + logs.map(function (l) {
            var opened = l.opened_at
              ...
              : (l.tracking_token ? '<span style="color:#9CA3AF;">not yet</span>' : '<span style="color:#CBD5E1;">—</span>');
