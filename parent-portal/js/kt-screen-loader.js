...
(function (w) {
  ...
  (function () {
    ...
    for (var i = 0; i < tags.length; i++) {
      ...
      if (path.indexOf('/js/') === -1) { continue; }
      already['/' + path.replace(/^\/+/, '')] = true;
      if (!pageStamp) {
        var m = /[?&]v=([^&]+)/.exec(raw);
        if (m) { pageStamp = m[1]; }
      }
  ...
  function inject(src) {
    ...
    /* Already in the page, at the version this deploy shipped. Loading it again can
       only re-run it from a different cache key — see above. */
    if (already[src]) {
      loaded[src] = Promise.resolve(true);
      return loaded[src];
    }

    loaded[src] = new Promise(function (resolve) {
      var s = document.createElement('script');
      /* The stamp from the page's own tags, NOT KT_VERSION. Once the static tags are
         removed and files really are lazy-only, this is what makes a deploy of one of
         them reach a browser that has the old one cached. */
      s.src = src + '?v=' + (pageStamp || w.KT_VERSION || 'rl1');
