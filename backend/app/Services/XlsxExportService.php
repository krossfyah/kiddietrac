...
final class XlsxExportService
{
    public function build(int $agencyId, string $title, array $columns, iterable $rowProvider, array $opts = []): array
    {
        ...
        // Generated-by + Private & Confidential footer
        $rowNum += 2;
        $by = $opts['generated_by'] ?? null;
        $sheet->setCellValue("A{$rowNum}", 'Generated by KiddieTrac' . ($by ? ' — ' . $by : '') . ' for ' . ($agency->name ?? '') . ' · ' . now()->format('F j, Y \a\t g:i A'));
        $sheet->mergeCells("A{$rowNum}:{$lastCol}{$rowNum}");
        $sheet->getStyle("A{$rowNum}")->getFont()->setSize(9)->setItalic(true)->getColor()->setRGB('9CA3AF');
        $rowNum++;
        $sheet->setCellValue("A{$rowNum}", 'PRIVATE & CONFIDENTIAL — Contains sensitive information. Handle securely and do not distribute without authorisation.');
        $sheet->mergeCells("A{$rowNum}:{$lastCol}{$rowNum}");
        $sheet->getStyle("A{$rowNum}")->getFont()->setSize(9)->setBold(true)->getColor()->setRGB('B91C1C');
