...
final class PayeeInvoiceController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $data = $request->validate([
            ...
            // Lines can arrive with the invoice rather than only being added afterwards.
            'lines' => ['nullable', 'array', 'max:50'],
            'lines.*.category' => ['required_with:lines', 'string', 'max:32'],
            'lines.*.description' => ['required_with:lines', 'string', 'max:300'],
            'lines.*.amount' => ['required_with:lines', 'numeric', 'min:-100000', 'max:100000'],
        ]);
        ...
        // Tax and any lines, then one derivation of the total from all of it.
        $this->ensureTaxColumns();
        $defaults = self::taxDefaults($agencyId);
        $applied = (bool) ($data['tax_applied'] ?? false);
        DB::table('payee_invoices')->where('id', $id)->update([
            'base_amount' => $amount,
            'tax_applied' => $applied,
            // An explicit rate wins; otherwise the agency's default fills in.
            'tax_rate' => $applied ? (float) ($data['tax_rate'] ?? $defaults['rate']) : null,
            'tax_label' => $applied ? (string) ($data['tax_label'] ?? $defaults['label']) : null,
        ]);

        if (! empty($data['lines'])) {
            $this->ensureLinesTable();
            foreach ($data['lines'] as $l) {
                if ((float) $l['amount'] === 0.0) { continue; }
                DB::table('payee_invoice_lines')->insert([
                    'payee_invoice_id' => $id,
                    'category' => strtolower((string) $l['category']),
        ...
        $this->ensureLinesTable();
        $total = $this->retotal($id);

        return response()->json(['ok' => true, 'id' => $id, 'amount' => $total, 'subtotal' => $amount], 201);
    }
    ...
    private function documentHtml(object $row): string
    {
        ...
        return '<!doctype html><html><head><meta charset="UTF-8"><style>'
            ...
            . (function () use ($row, $fmt) {
                // Subtotal and tax are only worth a line when there is something to
                // separate; a lone "Subtotal" identical to the total is noise.
                $sub = $row->subtotal ?? null;
                $tax = (float) ($row->tax_amount ?? 0);
                $out = '';
                if ($sub !== null && ($tax > 0 || (float) $sub !== (float) $row->amount)) {
                    $out .= '<div style="margin-top:12px;text-align:right;">Subtotal ' . e($fmt($sub)) . '</div>';
                }
                if ($row->tax_applied && $tax > 0) {
                    $out .= '<div style="text-align:right;">' . e((string) ($row->tax_label ?: 'Tax'))
                        . ' ' . e(rtrim(rtrim(number_format((float) $row->tax_rate, 2), '0'), '.')) . '% '
                        . e($fmt($tax)) . '</div>';
                } elseif (! $row->tax_applied) {
                    // Said out loud: a reader can tell zero-rated from forgotten.
                    $out .= '<div class="muted" style="text-align:right;font-size:11px;">No tax applied</div>';
                }
                return $out;
            })()
            . '<div class="total" style="text-align:right;">Total ' . e($fmt($row->amount)) . '</div></div>' 
    ...
    /** Tax columns, added on demand like the rest of this table. */
    private function ensureTaxColumns(): void
    {
        ...
        if (Schema::hasColumn('payee_invoices', 'tax_amount')) return;
        Schema::table('payee_invoices', function ($t) {
            $t->decimal('subtotal', 10, 2)->nullable()->after('base_amount');
            $t->boolean('tax_applied')->default(false)->after('subtotal');
            $t->decimal('tax_rate', 5, 2)->nullable()->after('tax_applied');
            $t->string('tax_label', 16)->nullable()->after('tax_rate');
            $t->decimal('tax_amount', 10, 2)->nullable()->after('tax_label');
        });
    }

    /** The agency's configured rate, which pre-fills but never overrides an explicit one. */
    public static function taxDefaults(int $agencyId): array
    {
        $row = DB::table('agencies')->where('id', $agencyId)->value('settings');
        $b = (json_decode((string) $row, true) ?: [])['billing'] ?? [];
        return [
            'rate' => isset($b['tax_rate']) ? (float) $b['tax_rate'] : 0.0,
            'label' => (string) ($b['tax_label'] ?? 'Tax'),
        ];
    }
    ...
    private function retotal(int $invoiceId): float
    {
        ...
        // Subtotal first, then tax on the whole of it — base AND lines, per the agreed
        // spec. Tax is rounded ONCE here: rounding per line and summing drifts by a cent
        // and the PDF stops adding up.
        $subtotal = round($core + $sum, 2);

        $this->ensureTaxColumns();
        $inv = DB::table('payee_invoices')->where('id', $invoiceId)->first(['tax_applied', 'tax_rate']);
        $taxAmount = 0.0;
        if ($inv && $inv->tax_applied && (float) $inv->tax_rate > 0) {
            $taxAmount = round($subtotal * ((float) $inv->tax_rate / 100), 2);
        }

        $total = round($subtotal + $taxAmount, 2);
        DB::table('payee_invoices')->where('id', $invoiceId)->update([
            'subtotal' => $subtotal,
            'tax_amount' => $taxAmount,
            // amount stays the GRAND total: every existing reader depends on that meaning.
            'amount' => $total,
            'updated_at' => now(),
        ]);
        return $total;
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        $this->ensureTaxColumns();
        $update = [];
        // Changing the tax is an edit like any other, so it passes the same paid/void
        // guard above rather than having a route of its own.
        if ($request->has('tax_applied')) {
            $applied = $request->boolean('tax_applied');
            $defaults = self::taxDefaults((int) $row->agency_id);
            $update['tax_applied'] = $applied;
            $update['tax_rate'] = $applied ? (float) ($data['tax_rate'] ?? $row->tax_rate ?? $defaults['rate']) : null;
            $update['tax_label'] = $applied ? (string) ($data['tax_label'] ?? $row->tax_label ?? $defaults['label']) : null;
        } elseif (isset($data['tax_rate'])) {
            $update['tax_rate'] = (float) $data['tax_rate'];
        }
